...
import { Video, Users, Clock, Play, Calendar } from 'lucide-react';
...
const LiveClassCard: React.FC<LiveClassCardProps> = ({
  ...
  useEffect(() => {
    const updateCountdown = () => {
      const now = new Date();
      const diff = liveClass.startTime.getTime() - now.getTime();
  ...
  return (
    <div
      className={cn(
        ...
        liveClass.isLive && 'ring-2 ring-destructive',
        className
      )}
    >
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="text-right">
            <p className="text-xs text-muted-foreground">
              {liveClass.isLive ? 'Happening now' : 'Starts in'}
            </p>
            <p className={cn(
              'text-lg font-bold',
